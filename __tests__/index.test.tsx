...
describe("App front-end tests", async () => {
  ...
  test("it can upload a file", async () => {
    ...
    const file = new File(['{"full": "full"}'], "test.json", { type: "application/json" })
  ...
  test("missing keys are marked with an x", async () => {
    ...
    const files = new File(['{"missing1": "", "missing2": "", "filled": "filled value"}'], "missing.json", { type: "application/json" })
    const uploadButton = screen.getByTestId("dropzone")
    await userEvent.upload(uploadButton, files)
    const missingKeyMarkedWithX = await screen.findAllByTestId("missing-key-missing1")
    const missingKeyMarkedWithX2 = await screen.findAllByTestId("missing-key-missing2")
